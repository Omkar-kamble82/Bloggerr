...
import { GetBlog,UpdateBlog } from "../firebase/functions";
...
const Updateblogpage = () => {
    ...
    const [blog, setBlog] = useState({title: "", image: "", category: "", username: "", profileimg: ""})
    ...
    const getblog = async () => {
        if (typeof id === "string"){
            ...
            setBlog({title: blogobj?.title, image: blogobj?.image, category: blogobj?.category, username: blogobj?.username, profileimg: blogobj?.profileImg})
            setContent(blogobj?.content)
        }
    }

    const Submit = async () => {
        try {
            await UpdateBlog({...blog, content}, id as string)
        ...
        <div>
            ...
            <div className="my-[50px] w-[90%] mx-[5%] sm:w-[80%] sm:mx-[10%]">
                <form>
                    ...
                    <input value={blog.title} onChange={(e) => {setBlog({...blog, title: e.target.value})}} type="text" className="border-[1px] border-[#c9c9c9] w-full p-2 outline-none" placeholder="Exploring the Wonders of Nature...."/>

                    <p className="text-[35px] mt-[15px] sm:mt-[10px] sm:text-[60px] font-semibold font">Blog: </p>
                    <ReactQuill value={content} onChange={setContent} className="editor" theme="snow" />
                    ...
                    <p className="text-rose-500 text-[12px] sm:text-[15px]">*The earlier picture would be used if you don't update.</p>

                    <p className="text-[35px] mt-[15px] sm:mt-[10px] sm:text-[60px] font-semibold font">Choose the Category: </p>
                    <div className="flex gap-5 flex-wrap">
                        <span><input checked={blog.category === "Adventure"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Adventure"}/><label className="text-[20px] font ml-[3px]">Adventure</label></span>
                        <span><input checked={blog.category === "Art"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Art"}/><label className="text-[20px] font ml-[3px]">Art</label></span>
                        <span><input checked={blog.category === "Technology"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Technology"}/><label className="text-[20px] font ml-[3px]">Technology</label></span>
                        <span><input checked={blog.category === "Science"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Science"}/><label className="text-[20px] font ml-[3px]">Science</label></span>     
                        <span><input checked={blog.category === "Food"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Food"}/><label className="text-[20px] font ml-[3px]">Food</label></span>     
                        <span><input checked={blog.category === "Cinema"} onChange={(e) => {setBlog({...blog, category: e.target.value})}} type="radio" name="cat" value={"Cinema"}/><label className="text-[20px] font ml-[3px]">Cinema</label></span>     
                    </div>               
                </form>
                <button onClick={Submit} className="px-5 font-semibold bg-[#2d2d2d] border-[#2d2d2d] text-[white] py-2 border-2 rounded-md mt-[25px] hover:opacity-90 duration-500 transition-all">Update post</button>
            </div>
        </div>
    )
}
